...
void WorldScopedMaps::init () {
    ...
    sx_ = height_pixel_ / dx_;  // scaling x
    sy_ = width_pixel_ / dy_;  // scaling y
    ox_ = width_pixel_ / 2.0;  // offset image space x
    oy_ = height_pixel_ / 2.0;  // offset image space y
    mx_ = min_x_ + dx_/2.0;  // visual image space x
    my_ = min_y_ + dy_/2.0;  // visual image space y
    double cr = cos ( rotation_ );
    double sr = sin ( rotation_ );
    ...
    cv::Matx<double, 3, 3 > R ( cr, -sr, 0, sr, cr, 0, 0, 0, 1 );  // rotation
