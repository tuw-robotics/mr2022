...
void WorldScopedMaps::init () {
...
#else
    dx_ = abs(max_x_ - min_x_);  // visual width
    dy_ = abs(max_y_ - min_y_);  // visual height
    ...
    mx_ = min_x_ + abs(max_x_ - min_x_) / 2;  // visual image space x
    my_ = min_y_ + abs(max_y_ - min_y_) / 2;  // visual image space y
