...
#else
    dx_ = width_pixel_; // visual width
    dy_ = height_pixel_; // visual height
    sx_ = dx_ / (max_x_ - min_x_); // scaling x
    sy_ = dy_ / (max_y_ - min_y_); // scaling y
    ox_ = dx_/2.; // offset image space x
    oy_ = dy_/2.; // offset image space y
    mx_ = (min_x_ + max_x_) / 2.; // visual image space x
    my_ = (min_y_ + max_y_) / 2.; // visual image space y
    
    cv::Matx<double, 3, 3 > Tw ( 1, 0, -mx_, 
                                 0, 1, -my_, 
                                 0, 0, 1 ); // translation visual space
