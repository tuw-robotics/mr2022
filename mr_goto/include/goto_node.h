...
public:
    ...
    void predictedPoseListener (); /// gets the current predicted pose
private:
    ...
    ros::Subscriber sub_path_;  /// Subscriber to the path planner path
    ros::Publisher pub_cmd_;    /// publisher for the motion commands
    std::shared_ptr<tf::TransformListener> tf_listener_;  /// listener to receive transformation messages -> to get the predicted pose
    ...
    void callbackPath (const nav_msgs::Path &); /// callback function to execute on incoming path data
